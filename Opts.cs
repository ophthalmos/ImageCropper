...
public partial class Opts : Form
{
    public int JpegQuality
    {
        get
        {
            ...
            {
                if (int.TryParse(cbxJPEGQuality.SelectedItem.ToString(), out var result)) { return result; }
            }
            return 90; // Fallback
        }
    }
    ...
    public string ImageEditorPath => tbxImageEditor.Text;

    public Opts(float currentRatio, int currentWidth, string imageName, string saveDir, long quality, bool closeOption, string imageEditor)
    {
        InitializeComponent();
        numericUpDown.Value = currentWidth;
        tbxImageName.Text = imageName;
        tbxTargetDir.Text = saveDir;
        tbxImageEditor.Text = imageEditor;
        if (currentRatio == 0.0f)
        {
            rBtnFree.Checked = true;  // RadioButtons setzen
        }
        else if (currentRatio == 1.0f)
        {
            rBtn1to1.Checked = true;
        }
        else if (Math.Abs(currentRatio - (2f / 3f)) < 0.01)
        {
            rBtn2to3.Checked = true;
        }
        else if (Math.Abs(currentRatio - (3f / 4f)) < 0.01)
        {
            rBtn3to4.Checked = true;
        }

        var index = -1;
        for (var i = 0; i < cbxJPEGQuality.Items.Count; i++)
        {
    ...
    private void BtnDefault_Paint(object sender, PaintEventArgs e)
    {
        ...
        var symbol = "↻";
        e.Graphics.TextRenderingHint = System.Drawing.Text.TextRenderingHint.ClearTypeGridFit;
        using Font iconFont = new("Segoe UI Symbol", 11F, FontStyle.Regular);
        SizeF textSize = e.Graphics.MeasureString(symbol, iconFont);
        var x = (btn.ClientRectangle.Width - textSize.Width) / 2;
        var y = (btn.ClientRectangle.Height - textSize.Height) / 2;
    ...
    private void BtnFileBrowser_Click(object sender, EventArgs e)
    {
        ...
        openFileDialog.Title = "Wählen Sie das Bildbearbeitungsprogramm aus";
        openFileDialog.Filter = "Anwendungen (*.exe)|*.exe|Alle Dateien (*.*)|*.*";
        openFileDialog.InitialDirectory = Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles);
        if (openFileDialog.ShowDialog() == DialogResult.OK)
        {
            tbxImageEditor.Text = openFileDialog.FileName;
        }
    }
}
